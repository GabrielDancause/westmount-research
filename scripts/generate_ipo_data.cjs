...
async function run() {
  ...
  for (const row of rows.slice(0, 50)) { // process top 50
  ...
  fs.writeFileSync(path.join(process.cwd(), 'data', 'ipo-data.json'), JSON.stringify(results, null, 2));
  console.log("Done. Saved to data/ipo-data.json");
}
